...
/*** TO BE DONE START ***/
	...
	if (connect(tcp_socket,(struct sockaddr *) ipv4, sizeof(*ipv4)) == -1)
...
/*** TO BE DONE START ***/
	if(!strncpy(answer, "OK",2))
		fail("Server response is not OK");
/*** TO BE DONE END ***/
...

	{
		...
		for(rep = 1; rep <= norep; ++rep) {
			...
			printf("Round trip time was %lg milliseconds in repetition %d/%d\n", current_time, rep, norep);
		}
