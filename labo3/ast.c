...
#define _GNU_SOURCE //Neededed to add strchrnul
...
/*** TO BE DONE START ***/
	char *path;
	if (!impl || !impl->pathname) {
		path = my_strdup(vt_lookup(sh_get_var_table(sh), "HOME"));		
	} else {
		path = my_strdup(impl->pathname);
	}
	if (chdir(path) == -1)
...
/*** TO BE DONE START ***/
	...
	if (dup2(from_fd, to_fd) == -1)
		fail_errno("Error on redirecting from from_fd to to_fd");
/*** TO BE DONE END ***/
}
...
/*** TO BE DONE START ***/
	pid = fork();
	if (pid == -1)
		fail_errno("Failed to fork ext_cmd");
/*** TO BE DONE END ***/
	if (pid == 0) {
		 ...
		 * la syscall fallisce. */
/*** TO BE DONE START ***/
		if (execve(executable, impl->args, vt_to_envp(sh_get_var_table(sh))) == -1)
			fail_errno("Failed to run ext_cmd");
/*** TO BE DONE END ***/
	}
	cleanup_and_return:
	free(executable);
...
/*** TO BE DONE START ***/
	i = pipe(pipes);
	if (i == -1)
		fail_errno("Failed to create pipe");
	//Set close-on-exec on the tow fd
	if (fcntl(pipes[0], FD_CLOEXEC) == -1) 
		fail_errno("errorn creating pipes[0]");
	if (fcntl(pipes[1], FD_CLOEXEC) == -1)
		fail_errno("errorn creating pipes[1]");
