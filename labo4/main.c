...
/*** TO BE DONE 3.0 START ***/
	if(chroot(www_root) == -1) {
		fail_errno("Failed to change root directory");
	}
...
/*** TO BE DONE 3.0 START ***/
	if(pthread_create(&thread_ids[i], NULL, (void*)client_connection_thread,  &connection_no[i]) == -1){
		fail_errno("Failed to create new thread");
	}
/*** TO BE DONE 3.0 END ***/
