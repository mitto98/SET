...
/*** TO BE DONE 3.1 START ***/
	...
	if(to_join[conn_no] != NULL){
		...
		debug(" ...Joined last response thread!\n");	
...
/*** TO BE DONE 3.1 START ***/
	...
	if(to_join[thrd_no] != NULL) {
		...
		debug("   ...joining thread with idx %ld (%ld)\n", i, thread_ids[i]);
		int pret;
		if(pret = pthread_join(thread_ids[i], NULL)) //Joining thread
			fail("Failed to join thread");
		pthread_mutex_lock(&threads_mutex);
		...
		debug(" ...no previous thread to join\n");
	}
...
/*** TO BE DONE 3.1 START ***/
	debug(" ... enqueued current thread in to_join queue (tidx=%d)\n", new_thread_idx);
