...
/*** TO BE DONE 3.0 START ***/
			debug("    ... send_response(%d, %s): opened file %d\n", response_code, HTML_501, fd);
			mime_type = my_strdup(HTML_mime);
			debug("    ... send_response(%d, %s): got mime type %s\n", response_code, HTML_501, mime_type);
			stat_p = &stat_buffer;
			if (stat(HTML_501, stat_p))
				fail_errno("Failed stat HTML 501");
			file_size = stat_p->st_size;
			file_modification_time = stat_p->st_mtim.tv_sec;
/*** TO BE DONE 3.0 END ***/
...
/*** TO BE DONE 3.0 START ***/ 
			...
			//incApache solo un cookie mandare :-/ Ugh!
			if(cookie!=-1)
				sprintf(http_header, "%s\r\nset-cookie: UserID=%d", http_header, cookie);
/*** TO BE DONE 3.0 END ***/
...
/*** TO BE DONE 3.0 END ***/
		...
		if (access(filename, R_OK) != 0 || stat(filename, stat_p) < 0) {
			...
			if (MethodIsConditional(http_method)) {
				 ...
				 ***/
/*** TO BE DONE 3.0 START ***/
				if(my_timegm(&since_tm) >= stat_p->st_mtim.tv_sec) {	//METHOD_NOT_CHANGED if file not changed
					debug("since_tm: %ld, last_modification: %ld\n", my_timegm(&since_tm), stat_p->st_mtim.tv_sec);				
					http_method = METHOD_NOT_CHANGED;
				} else 
					http_method &= ~METHOD_CONDITIONAL;	//Remove METHOD_CONDITIONAL flag id date changed
/*** TO BE DONE 3.0 END ***/
